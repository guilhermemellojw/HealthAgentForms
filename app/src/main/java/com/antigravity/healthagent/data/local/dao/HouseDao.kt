...
interface HouseDao {
    ...
    @Query("SELECT DISTINCT data FROM houses WHERE agentName = :agentName")
    ...
    @Query("UPDATE houses SET data = :newDate WHERE data = :oldDate AND agentName = :agentName")
    suspend fun updateHousesDate(oldDate: String, newDate: String, agentName: String)


    @Query("DELETE FROM houses WHERE data = :date AND agentName = :agentName")
