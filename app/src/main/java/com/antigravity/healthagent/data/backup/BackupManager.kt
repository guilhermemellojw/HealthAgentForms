...
class BackupManager @Inject constructor() {
    ...
    fun exportData(context: Context, uri: Uri, data: BackupData) {
        ...
        try {
            ...
            // 1. Serialize to a temporary encrypted file first
            ...
            // 2. Only if successful, copy the encrypted bytes to the final destination URI
            ...
            AppLogger.e("BackupManager", "Erro ao exportar dados para URI", e)
            throw e 
        } finally {
    ...
    fun exportToFile(context: Context, file: java.io.File, data: BackupData) {
        ...
        try {
            ...
            AppLogger.e("BackupManager", "Erro ao exportar para arquivo", e)
            throw e
        } finally {
            if (tempFile.exists()) try { tempFile.delete() } catch(e: Exception) {}
        }
    ...
    fun importData(context: Context, uri: Uri): BackupData {
        val tempFile = java.io.File(context.cacheDir, "backup_import_temp.json")
        try {
            ...
            // Try to decrypt and read using EncryptedFile
            return try {
                val masterKey = MasterKey.Builder(context)
                    .setKeyScheme(MasterKey.KeyScheme.AES256_GCM)
                    .build()
                ...
                encryptedFile.openFileInput().use { decryptedStream ->
                    parseFromStream(decryptedStream)
                }
            } catch (e: Exception) {
                AppLogger.w("BackupManager", "Failed to decrypt backup, falling back to plain JSON: ${e.message}")
                // Fallback to plain JSON
                tempFile.inputStream().use { plainStream ->
                    parseFromStream(plainStream)
                }
            }
        } finally {
            if (tempFile.exists()) try { tempFile.delete() } catch(e: Exception) {}
    ...
    private fun parseFromStream(stream: java.io.InputStream): BackupData {
        return try {
                ...
                if (firstChar == '{') {
                    ...
                    while (jsonReader.hasNext()) {
                        val name = try { jsonReader.nextName() } catch (e: Exception) { 
                            AppLogger.e("BackupManager", "JSON Truncated at root name: ${e.message}")
                            wasTruncated = true
                            break 
                        }
                        AppLogger.i("BackupManager", "Importing section: $name")
                        when (name) {
                            "houses" -> {
                                ...
                                try {
                                    ...
                                    AppLogger.e("BackupManager", "JSON Truncated in houses array: ${e.message}")
                                    wasTruncated = true
                                }
                            ...
                            "dayActivities", "day_activities" -> {
                                ...
                                try {
                                    ...
                                    AppLogger.e("BackupManager", "JSON Truncated in activities array: ${e.message}")
                                    wasTruncated = true
                                }
                                try { if (jsonReader.peek() != JsonToken.END_ARRAY) wasTruncated = true else jsonReader.endArray() } catch (e: Exception) { wasTruncated = true }
                            }
                    ...
                    try {
                        ...
                        AppLogger.e("BackupManager", "JSON Truncated in legacy houses array: ${e.message}")
                        wasTruncated = true
                    }
                    try { if (jsonReader.peek() != JsonToken.END_ARRAY) wasTruncated = true else jsonReader.endArray() } catch (e: Exception) { wasTruncated = true }
                }
            } catch (e: Exception) {
                AppLogger.e("BackupManager", "Erro ao processar JSON houses", e)
                wasTruncated = true
            }
            ...
            AppLogger.e("BackupManager", "Erro de formato no backup", e)
